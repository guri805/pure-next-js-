"use client";
import { fetchCategories, handleDeleteProduct, handleFetchProduct, handleUpdateProduct, } from "../../lib/action";
...
const AllProduct = () => {
  ...
  const handleUpdateProductClick = async (e) => {
    ...
    try {
      const updateProduct = await handleUpdateProduct(editProduct._id, updatedProduct);
      if (updateProduct.success) {
        setEditProduct(null);
        setUpdatedProduct({ name: "", description: "", price: "", categoryId: "" });
        setShowModal(false);
        setProducts((prev) =>
          prev.map((product) =>
